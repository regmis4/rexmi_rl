...
class Go2wFlatEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.actions.hip_pos = JointPositionActionCfg(
            ...
            scale=0.3,                        # ±0.3 rad — lateral balance and steering
            use_default_offset=True,          # relative to default stance
        )
        ...
        # Leg joint deviation: soft bias toward default stance for all leg joints.
        #
        # Weight -0.05 is deliberately weak — Phase 6 discovered that a stronger
        # penalty (-0.2) suppressed calf extension needed to lift wheels above step
        # edges.  -0.05 still penalises extreme spreading while preserving the full
        # range of motion needed for climbing, turning, and lateral stepping.
        # Covers all leg joints: hip, thigh, and calf.
...
class Go2wRoughEnvCfg(Go2wFlatEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.terrain.terrain_generator = TerrainGeneratorCfg(
            ...
            size=(8.0, 8.0),
            border_width=20.0,
            num_rows=10,    # 10 curriculum difficulty levels (row 0 = flat, row 9 = hardest)
